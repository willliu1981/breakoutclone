...
public class Physics {
		...
		gameObjects.forEach(g -> {
			...
			colliders.forEach(c -> {
				if (c instanceof Rectangle) {
					...
					Point p1 = Locations.translateToWorldLocation(c,
							new Point());
					Point p2 = Locations.translateToWorldLocation(c,
							new Point(rect.getWidth(), 0));
					Point p3 = Locations.translateToWorldLocation(c,
							new Point(rect.getWidth(), rect.getHeight()));
					Point p4 = Locations.translateToWorldLocation(c,
							new Point(0, rect.getHeight()));
	...
	static public boolean Raycast(Ray ray, Line targetLine, RayLineCastHit hit,
			double distance) {

		Point rayIntersectPoint = rayIntersectPoint(ray.getP0(), ray.getP1(),
				targetLine.p0, targetLine.p1);
